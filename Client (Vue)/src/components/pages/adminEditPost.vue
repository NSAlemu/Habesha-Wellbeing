      ...
      <b-col cols="12" lg="8">
        <b-form v-if="editOptionsSelected === 'editMode'" @submit="onSubmit">
          ...
          <b-form-group id="input-group-1" label="Title" label-for="input-1">
            <b-form-input id="input-2" v-model="postBody.title" required v-text="jkhkjhh"
  ...
  export default {
    ...
    async mounted() {
      try {
        ...
        this.tempPost = this.postBody.post;
      } catch (err) {
    ...
    methods: {
      ...
      postNow: async function () {
        ...
        var saved = ""
        try {
          saved = await PostService.updatePost(this.$route.params.PostID,newPost);
          if (saved === 'true') {
